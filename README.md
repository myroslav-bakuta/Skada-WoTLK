...
* **Pets and guardians:** damage done by a shaman's fire or earth elemental was frequently credited to nobody, so a fight driven by the elemental alone showed an empty window until the shaman attacked in person. Four separate faults contributed. The `SPELL_SUMMON` handler rejected a summon whose source was itself a guardian, which is exactly the shaman's `player > totem > elemental` chain. The combat log does not order those two summons, and when `totem > elemental` arrived first the elemental stayed owned by the totem, since ownership was only ever collapsed at insertion time; pets owned by a pet are now re-pointed once the real owner becomes known. The one second tick that closes a segment consulted `IsGroupInCombat`, which walks unit ids only and therefore cannot see a guardian, ending the segment while the elemental was still swinging. Finally, the tentative combat window required five events within one second, a threshold a guardian attacking roughly every two seconds can never reach on its own; a hit from a group member's pet now starts the fight immediately, while non pet events remain as conservative as before.
...
* Version declared consistently in all three addons, currently 1.9.4. The two plugins previously had no version field at all.
