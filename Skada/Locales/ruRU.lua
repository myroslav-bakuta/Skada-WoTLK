...
L["opt_tweaks_stalecombat_warn"] = "|cffffbb00Внимание:|r значение не может быть меньше %d с, заданных в разделе \"%s\". Если поднять то значение, это поднимется вместе с ним."
...
L["opt_tweaks_debuglog_help"] = "Журнал хранится в памяти и попадает в файл сохранённых переменных только при перезагрузке интерфейса или выходе из игры.\nХранятся последние 3 сессии, до 50000 строк каждая, чего хватает на целый рейдовый вечер.\nКоманды: /skada log on | verbose | off | clear"
