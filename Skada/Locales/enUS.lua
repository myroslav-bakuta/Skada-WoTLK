...
L["Stuck Combat Guard"] = true
L["opt_tweaks_stalecombat_desc"] = "The client can leave a group member flagged as fighting long after everything is dead, which keeps the segment running."
L["Idle Time"] = true
L["opt_tweaks_stalecombattime_desc"] = "How long the combat log must stay quiet, with you out of combat, before the segment is ended."
L["Segment End Delay"] = true
L["opt_tweaks_combatgrace_desc"] = "How long the whole group has to stay out of combat before the fight counts as over."
L["opt_tweaks_combatgrace_slider_desc"] = "Seconds of quiet before the segment ends. Zero ends it at the first quiet moment."
L["opt_tweaks_stalecombat_warn"] = "\n|cffffbb00Note:|r this cannot go below %d seconds, the value set in \"%s\". Raising that one raises this along with it."
L["Stopped Segment Timeout"] = true
L["opt_tweaks_autostopclose_desc"] = "A segment frozen after a boss dies records nothing, so it is closed on a timer instead of waiting for the group to leave combat."
L["opt_tweaks_autostopclose_slider_desc"] = "Seconds to wait before closing a segment that was stopped automatically."
L["opt_tweaks_autostopclose_note"] = "\n|cffffbb00Note:|r a segment you stopped yourself is never closed by this and waits for you to resume it."
L["Split Late Boss Pulls"] = true
L["Trash Time"] = true
L["opt_tweaks_bosssplit_desc"] = "A trash pull that runs into a boss used to hand the boss all of that trash, which stretches the fight and drags everyone's numbers down. Once a segment has been running on other targets for a while and only then meets a boss, Skada closes that segment and opens a clean one for the boss."
L["opt_tweaks_bosssplittime_desc"] = "How long a segment must have been running on other targets before a boss gets a segment of its own."
L["opt_tweaks_bosssplit_note"] = "\n|cffffbb00Note:|r only a segment that started on something else can be split, so a boss you pull directly always keeps its own segment."
L["Drop Failed Attempts"] = true
L["opt_tweaks_dropwipes_desc"] = "Failed attempts on a boss are saved the same as a kill and stay until you delete them. Turn this on to have a successful kill drop the earlier failed attempts on that same boss."
L["opt_tweaks_dropwipes_note"] = "\n|cffffbb00Note:|r a released segment is not deleted at once, it just loses its reserved place and is trimmed like any other once the list is full. Only attempts on the boss you killed are released. Needs \"%s\"."
L["Minimum Attempt DPS"] = true
L["opt_tweaks_minattemptdps_desc"] = "The damage per second the raid has to deal before a pull counts as a real attempt worth saving the segment for. A pet that pulled a boss, or a boss that evaded at once, leaves a segment carrying the boss name and almost nothing else."
L["opt_tweaks_minattemptdps_note"] = "\n|cffffbb00Note:|r a kill is always saved, whatever damage it dealt. Zero turns this damage check off, leaving only the other rule: a pull the raid never entered combat for is never saved."
L["Name Segments By Main Enemy"] = "Trash Pull Segment Name"
L["opt_tweaks_trashrename_desc"] = "A segment takes its name from the first mob the combat log happens to name (in a raid that is often an ordinary mob out of the trash pack). With this on the segment is named after the mob the raid actually fought, rather than after a boss."
L["opt_tweaks_trashrename_note"] = "\n|cffffbb00Note:|r boss fights already carry the right name and are never renamed."
L["Debug Log"] = true
L["Verbose"] = true
L["Clear"] = true
L["opt_tweaks_debuglog_desc"] = "Records what Skada decides about segments, bosses, the group roster and pets.\nMeant for reporting a problem: turn it on, play, then reload the interface."
L["opt_tweaks_debuglog_enable_desc"] = "Start recording."
L["opt_tweaks_debuglog_verbose_desc"] = "Also record raw combat log events at the start of every segment. Much larger, use it for one fight rather than a whole raid."
L["opt_tweaks_debuglog_clear_desc"] = "Throw away everything recorded so far and start a fresh session."
L["opt_tweaks_debuglog_status"] = "Recording: %s      Verbose: %s      Lines: %d      Sessions: %d"
L["opt_tweaks_debuglog_help"] = "\nThe log is kept in memory and only reaches the saved variables file on a reload or logout.\nThe last 3 sessions are kept, up to 50000 lines each, which is room for a whole raid night.\nSlash commands: /skada log on | verbose | off | clear"
L["Modes Icons"] = true
